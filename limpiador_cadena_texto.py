# -*- coding: utf-8 -*-
"""limpiador_cadena_texto.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1fiA_Q55ewOU2oV210iyae9Hrx7xPhXVg
"""
...
df_casa = pd.read_csv('casa.csv', error_bad_lines=False )
df_casa.head()

df_casa['detalle_adicional;;;;;'][0]

"""BORRARMOS DUPLICADOS"""

df_casa = df_casa.drop_duplicates()

"""BORRO FILAS  SI TENGO NANG EN METRAJECONTRUC"""
    ...
    for prop in fetaures:
        if match_property(fetaures,['comercial','zona comercial']):
           return 1
            ...
            return 0
def get_areasocial (fetaures):
    for prop in fetaures:
        if match_property(fetaures,['social','cancha','gimnasio']):
           return 1
        else:
            return 0      
def get_urbanizacion (fetaures):
    for prop in fetaures:
        if match_property(fetaures,['urbanizacion']):
           return 1
        else:
            return 0  
...
df_casa['areasocial'] = df_casa['detalle_adicional;;;;;'].apply(get_areasocial)
df_casa['urbanizacion'] = df_casa['detalle_adicional;;;;;'].apply(get_urbanizacion)

df_casa['baños'] = df_casa['baños'].astype(str)
df_casa = df_casa[df_casa['baños'].str.strip() != '']

df_casa['piscina'] = df_casa['piscina'].astype(str)
df_casa = df_casa[df_casa['piscina'].str.strip() != '']

df_casa['ubicacion'] = df_casa['ubicacion'].astype(str)
df_casa = df_casa[df_casa['ubicacion'].str.strip() != '']

df_casa = df_casa.dropna(subset=['ubicacion'])

df_casa = df_casa.dropna(subset=['metrajeconstruc'])

df_casa['ubicacion'] = df_casa['ubicacion'].replace('nan', pd.NA)
df_casa = df_casa.dropna(subset=['ubicacion'])

df_casa['zona'] = df_casa['zona'].replace('NaN', pd.NA)
df_casa = df_casa.dropna(subset=['zona'])

df_casa['metrajeterreno'] = df_casa['metrajeterreno'].replace('NaN', pd.NA)
df_casa = df_casa.dropna(subset=['metrajeterreno'])

df_casa.head(125)

df_casa.drop(['detalle_adicional;;;;;'], axis=1, inplace=True)

df_casa.to_csv('casa_modificado.csv', index=False)